...
struct BMPHeader {
  ...
  void write(std::ofstream &file) const {
    ...
    file.write(reinterpret_cast<const char *const>(&type), sizeof(type[0]) * 2);
    file.write(reinterpret_cast<const char *const>(this), sizeof(BMPHeader));
  }
};
...
void WriteBmp(const std::wstring &filename, const int32_t &height,
  ...
  for (int32_t i = 0; i < height; i++) {
    for (int32_t j = 0; j < width; j++) {
      ...
      bmp_bitmap[height - 1 - i][j] = {bitmap[id + 2], bitmap[id + 1],
                                       bitmap[id]};
    }
  }
